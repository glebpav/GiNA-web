...
def optimize(
    ...
    knots_force_ratios = np.ones((len(A), len(A)))
    ...
    for iteration in range(iterations):
        ...
        for i in range(len(A)):
            ...
            for idx1, connected_point_idx in enumerate(A[i]):
                if knots_force_ratios[i][connected_point_idx] != 1:
                    # print(f"{(idx1 + 1)}/{len(A[i])}")
                    print(
                        f"{i} - {knots_force_ratios[i][connected_point_idx]} - {np.linalg.norm(knots_force_ratios[i][connected_point_idx] * (-2) * (1 - l0 * target_edge_ratios[i][connected_point_idx] / distance[connected_point_idx]) * (pos[i] - pos[connected_point_idx]))}")
                    pass
                    # print(target_edge_ratios[i][connected_point_idx])

                elastic_force += (
                        (-2) * (1 - l0 * target_edge_ratios[i][connected_point_idx] / distance[connected_point_idx])
                        * (pos[i] - pos[connected_point_idx])
                )

            repulsion_force = (np.power(distance, -3) * delta).sum(axis=1)
...
def run_script(sequence):
    pos, adj_list = compute_graph(sequence)
